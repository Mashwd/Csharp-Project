...
namespace Superstore_Management_System.Presentation_Layer.Cashier
{
    public partial class CashierHome : Form
    {
        ...
        private void CashierHome_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                int quantity = new ProductDataAccess().ValidateQuantity(charts[i].Product);
                quantity = quantity + charts[i].Quantity;
        ...
        private void CashierHome_Load(object sender, EventArgs e)
        {
            ...
            discountLabel.Text = "Discount: 00.00 Tk."; 
        }
        ...
        private void addItemBunifuButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else if (res == "Unblock")
                {
                    ...
                    chart.Discount = chart.Price * new ProductDataAccess().discountAmount(chart.Product) / 100;
                    ...
                    {
                        double temp = new ChartDataAccess().SumOfChart() - new ChartDataAccess().DiscountOfChart();
                        priceLabel.Text = "Total Price: " + temp + ".00 Tk.";
                        discountLabel.Text = "Discount: " + new ChartDataAccess().DiscountOfChart() + ".00 Tk.";
                    }
                    else
                    {
                        priceLabel.Text = "Total Price: 00.00 Tk.";
        ...
        private void PaidButton_Click(object sender, EventArgs e)
        {
            if (new ChartDataAccess().IsEmpty() == true)
            {
                ...
                {
                    if (givenAmountTextBox.Text.All(Char.IsNumber) == false)
                    {
                        MessageBox.Show("Invalid amount in given amount.");
                    }
                    else
                    {
                        double temp = new ChartDataAccess().SumOfChart();
                        if (temp <= Convert.ToDouble(givenAmountTextBox.Text))
                        {
                            List<Chart> charts = new List<Chart>();
                            ...
                            transition.Time = DateTime.Now.ToString("yyyy'-'MM'-'dd' 'HH':'mm':'ss");
